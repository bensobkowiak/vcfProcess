...
vcfProcess = function(inputfile,outputfile,samples2remove=NULL,indelProcess=FALSE,DP_low=5,lowqual=20,hetProp=0.9,misPercent=80,repeatfile=NULL,excludeMix=FALSE){
  ...
  b<-as.integer(vcf[,6]) < lowqual
  lowqual_mat<-vcf[which(b=='TRUE'),]
  colnames(lowqual_mat)<-c(head_start,names)
  vcf<-vcf[which(b == 'FALSE'),]
  ...
  g<-do.call(rbind,d)
  output<-output[which(g=='FALSE'),] 
  vcf<-vcf[which(g=='FALSE'),]
  ...
  if (is.null(repeatfile)==FALSE){
    ...
    rep<-cbind(vcf[which(q == 'TRUE'),2],vcf[which(q == 'TRUE'),4],output[which(q=='TRUE'),])
    colnames(rep)<-c("Position","Reference",names)
    output<-output[which(q == 'FALSE'),]
    vcf<-vcf[which(q == 'FALSE'),]
    if (nrow(rep)!=0){
